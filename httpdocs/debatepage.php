...
<?php
$result = mysql_query("SELECT arguments.uid, arguments.aid, arguments.argument, arguments.rating, users.username, users.imagelocation  FROM arguments LEFT JOIN users ON arguments.uid=users.uid WHERE did = '$_GET[did]' AND side = 0 ORDER BY rating DESC");
while($row = mysql_fetch_array($result))
  {
