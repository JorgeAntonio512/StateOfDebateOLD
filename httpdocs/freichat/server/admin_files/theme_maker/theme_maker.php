    ...
    foreach ($arr[0] as $i => $x){
        ...
        foreach ($rules as $strRule){
            if (!empty($strRule)){
                ...
                if(isset($rules_arr[trim($rule[0])])){
                    $rules_arr[trim($rule[0])."_DUPLICATE_".(++$duplicate)] = trim($rule[1]);
                }else{
    ...
    public function save_style_changes(){
      ...
      foreach($css_array as $css_key => $css_values) {
          ...
          foreach ($css_values as $css_property => $css_value) {
              $pos = strpos($css_property, "_DUPLICATE_");
              
              if($pos !== FALSE) {
                  $css_property = substr($css_property, 0 ,$pos);
              }
