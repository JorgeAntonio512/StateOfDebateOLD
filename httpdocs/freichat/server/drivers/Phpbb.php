...
/*
        ...
        $result = $this->x_config;
        
        $ext = substr(strrchr($avatar, '.'), 1);
        
        foreach($result as $res){
            
            if($res['config_name'] == 'avatar_salt') {
                $prefix = $res['config_value'] . '_';                
            }else if($res['config_name'] == 'avatar_path'){
                $image_dir = $res['config_value'];                
            }
	...
	$avatar_url = $root_path . $image_dir . '/' . $prefix . $this->user_id . '.' . $ext;
        
*/
        
        $root_path = str_replace($this->to_freichat_path, "", $this->url);
    
        $avatar_url = $root_path."download/file.php?avatar=".$avatar;
